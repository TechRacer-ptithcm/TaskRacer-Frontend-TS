        ...
        <div className="flex items-center gap-4 space-y-2 text-sm">
          ...
          <div className="flex w-full flex-col">
            <div className="flex items-center justify-between gap-4">
              <span className="capitalize">{task?.start ? formatDate(task.start) : "Thứ hai, 15 tháng 4"}</span>
              <div className="flex items-center gap-x-2">
                <TimePicker 
                  value={task?.start ? new Date(task.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }) : ""} 
                  onChange={(value) => {
                ...
                <span>-</span>
                <TimePicker 
                  value={task?.end ? new Date(task.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false }) : ""} 
                  onChange={(value) => {
                    if (!task?.end) return;
        ...
        <div className="flex justify-center gap-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="z-[9999] w-40">
              ...
              <DropdownMenuItem onClick={() => handleUpdateTask({ status: "CANCELED" })}>
                Đã hủy
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
