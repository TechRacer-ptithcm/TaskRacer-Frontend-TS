...
export default function TaskManagement() {
  ...
  const handleAddTag = () => {
    ...
    setTasks(
      tasks.map((task) => (task.id === selectedTaskId ? { ...task, tags: [...(task.tags || []), newTag] } : task)),
    )
    ...
    setTasks(
      tasks.map((task) =>
        task.id === selectedTaskId
          ? {
              ...task,
              subtasks: [...(task.subtasks || []), { id: `sub${Date.now()}`, title: newSubtask, completed: false }],
            }
          : task,
      ),
    )
      ...
      <div className="w-full border-r md:w-1/3 lg:w-1/4">
        <div className="p-4">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="rounded-full bg-gray-100 px-2 py-1 text-sm text-gray-600">{tasks.length}</span>
          </div>

          <Button variant="ghost" className="mb-4 flex w-full items-center justify-start gap-2 text-gray-500">
            <Plus size={16} />
            ...
            {tasks.map((task) => (
              <div
                ...
                className={`cursor-pointer rounded-md p-3 ${
                  selectedTaskId === task.id ? "bg-gray-100" : "hover:bg-gray-50"
                }`}
      ...
      {selectedTask && (
        <div className="flex-1 p-4">
          ...
          <div className="mb-4 grid grid-cols-2 gap-4">
            ...
            <div>
              <h3 className="mb-2 text-sm font-medium text-gray-700">Due Date</h3>
              <Input value={selectedTask.dueDate || ""} className="h-8" />
            </div>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isAddingTag ? (
                <div className="flex items-center gap-1">
                  ...
                  />
                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => setIsAddingTag(false)}>
                    <X size={14} />
                  </Button>
                </div>
              ) : (
                <Button variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => setIsAddingTag(true)}>
                  + Add Tag
                </Button>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {isAddingSubtask && (
                <div className="flex items-center gap-2 pl-2">
                  ...
                  />
                  <Button size="sm" variant="ghost" className="h-8 w-8 p-0" onClick={() => setIsAddingSubtask(false)}>
                    <X size={16} />
                  </Button>
                </div>
              )}
          ...
          <div className="mt-auto flex justify-between">
            <Button variant="outline" className="text-gray-500" onClick={handleDeleteTask}>
              Delete Task
            </Button>
            <Button className="bg-yellow-400 text-black hover:bg-yellow-500" onClick={handleSaveChanges}>
              Save changes
            </Button>
          </div>
        </div>
